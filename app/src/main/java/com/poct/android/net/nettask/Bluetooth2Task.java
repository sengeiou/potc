...
public class Bluetooth2Task extends NetTask {
    ...
    public Bluetooth2Task(String url, Handler mHandler, int successEvent, int failEvent, Context mContext, String commend, int count, int count2,Object object) {
        ...
        try {
            if ( BluetoothSetManager.getInstance().netBluetoothDevice != null) {
                ...
                if (data.length() > 0 && data.contains(commend)) {
                    TempGet tempGet = (TempGet) item;
                    get = NetUtils.praseData(data, commend,tempGet.mTempid);
                }
