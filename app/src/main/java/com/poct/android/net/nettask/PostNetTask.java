...
public class PostNetTask extends NetTask {
	...
	public PostNetTask(String url, Handler mHandler, int successEvent, int failEvent, Context mContext, RequestBody formBody) {
	...
	public void run() {
		...
		if (NetUtils.checkNetWorkState(PoctApplication.mApp)) {
			String result = "";
			result = NetUtils.getInstance().post(mUrl, formBody);
			if (result == null) {
				if (mHandler != null)
