...
public class DataPrase {
    ...
    public static void praseGetReport(String text, ArrayList<Report> reports,ArrayList<Report> reports2) {
        String regEx = "[|]";
        Pattern p = Pattern.compile(regEx);
        Matcher m = p.matcher(text);
        String a = m.replaceAll(",").trim();
...
//        report.mGod = data[4];        report.mGod = data[4];
        ...
        report.time = data[8];
        if(checkReport(reports,report) == false && checkReport(reports2,report) == false)
        reports.add(report);
    }
...
//        seriesReport.mTime = data;
        ...
        try {
            ...
            for(int i = 0 ; i < ja.length() ; i++)
            {
                ...
                if(tempGet.realcount != -1)
                {
                    ...
                    {
                        if(reports.get(j).tempId.equals(tempGet.mTempid) && type.equals(reports.get(j).type))
                        {
                            if(count >  0)
                            {
                                count--;
    ...
    public static boolean hasTempid(String tempjson) {
        try {
            JSONObject jsonobject;
            JSONArray ja1;
            JSONArray ja2;
            ...
            if(jsonobject.has(EquipMentManager.DEVICE_POTC)) {
                ja1 = jsonobject.getJSONArray(EquipMentManager.DEVICE_POTC);
            }
            else
            {
                ja1 = new JSONArray();
            ...
            if(jsonobject.has(EquipMentManager.DEVICE_FADA)) {
                ja2 = jsonobject.getJSONArray(EquipMentManager.DEVICE_FADA);
            }
            else
            {
                ja2 = new JSONArray();
            }
            if((ja1.length()+ja2.length()) > 0)
            {
                return true;
            }
            else
    ...
    public static String addTempid(String old,String id,String type) {
        ...
        try {
            ...
            for(int i = 0 ; i < ja.length() ; i++)
            {
                ...
                int tid = Integer.valueOf(tjo.getString("tempid"));
                int addid = Integer.valueOf(id);
                if(tid > addid)
                {
                    JSONObject jo = new JSONObject();
                    jo.put("tempid",id);
            ...
            if(nja.length() == ja.length()) {
                JSONObject jo = new JSONObject();
                jo.put("tempid",id);
                jo.put("count",-1);
                nja.put(jo);
            }
            jsonobject.put(type,nja);
            return jsonobject.toString();
        } catch (JSONException e) {
            e.printStackTrace();
            return "";
    ...
    public static String addTempDbData(String oldjson, String reportid, Context context,String recordid) {
        ...
        DBHelper.getInstance(context).saveTempId(recordid,ja2.toString());
    ...
    public static boolean hasTempitem(String id,ArrayList<Report> reports) {
        boolean has = false;
        for(int i = 0 ; i < reports.size() ; i++) {
            if(reports.get(i).tempId.equals(id))
            {
                has = true;
                break;
            }
    ...
    public static String praseReportJson(SeriesReport seriesReport) {
        try {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("mReportId",seriesReport.mReportId);
            jsonObject.put("mPId",seriesReport.mPId);
            jsonObject.put("mAccountId",seriesReport.mAccountId);
            jsonObject.put("mName",seriesReport.mName);
            jsonObject.put("mSex",seriesReport.mSex);
            jsonObject.put("mAge",seriesReport.mAge);
            jsonObject.put("mSender",seriesReport.mSender);
            jsonObject.put("mSendTime",seriesReport.mSendTime);
            jsonObject.put("mApprover",seriesReport.mApprover);
            jsonObject.put("mCNum",seriesReport.mCNum);
            jsonObject.put("mHNum",seriesReport.mHNum);
            jsonObject.put("mDepart",seriesReport.mDepart);
            jsonObject.put("mBad",seriesReport.mBad);
            jsonObject.put("mDes",seriesReport.mDes);
            jsonObject.put("mMemo",seriesReport.mMemo);
            jsonObject.put("tempId",seriesReport.tempId);
            jsonObject.put("mApproveTime",seriesReport.mApproveTime);
            jsonObject.put("isUpdate",seriesReport.isUpdate);
            jsonObject.put("isApprove",seriesReport.isApprove);
            jsonObject.put("updatacount",seriesReport.updatacount);
            JSONArray ja = new JSONArray();
            jsonObject.put("report",ja);
    ...
    public static SeriesReport getJsonSerieaReport(String json) {
        SeriesReport seriesReport = new SeriesReport();
        try {

            XpxJSONObject jsonObject = new XpxJSONObject(json);
            seriesReport.mReportId = jsonObject.getString("mReportId");
            seriesReport.mPId = jsonObject.getString("mPId");
            seriesReport.mAccountId = jsonObject.getString("mAccountId");
            seriesReport.mName = jsonObject.getString("mName");
            seriesReport.mSex = jsonObject.getString("mSex");
            seriesReport.mAge = jsonObject.getString("mAge");
            seriesReport.mSender = jsonObject.getString("mSender");
            seriesReport.mSendTime = jsonObject.getString("mSendTime");
            seriesReport.mApprover = jsonObject.getString("mApprover");
            seriesReport.mCNum = jsonObject.getString("mCNum");
            seriesReport.mHNum = jsonObject.getString("mHNum");
            seriesReport.mDepart = jsonObject.getString("mDepart");
            seriesReport.mBad = jsonObject.getString("mBad");
            seriesReport.mDes = jsonObject.getString("mDes");
            seriesReport.mMemo = jsonObject.getString("mMemo");
            seriesReport.tempId = jsonObject.getString("tempId");
            seriesReport.mApproveTime = jsonObject.getString("mApproveTime");
            seriesReport.isUpdate = jsonObject.getBoolean("isUpdate",false);
            seriesReport.isApprove = jsonObject.getString("isApprove");
            seriesReport.updatacount = jsonObject.getInt("updatacount",0);
            XpxJSONArray ja = jsonObject.getJSONArray("report");
            for(int i = 0 ; i < ja.length() ; i++) {
                ...
                seriesReport.mReports.add(report);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
